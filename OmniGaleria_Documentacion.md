...
>
> **Versión documentada: v2.11.2** · Idioma de trabajo del proyecto: **español**.
...
### 11. Recorrido guiado por puntos (v2.11.0) — ✅
Reemplaza por completo el sistema de «ruta» anterior por un editor de recorrido más intuitivo, pensado como **presentación / slides** automática. Funciona en el editor, en el modo **Recorrido** y en el **HTML exportado**.

**Modelo de datos.** Se reutiliza `plan.tourNodes`: una lista ordenada de puntos, cada uno de tipo **`dest`** (una **posición**: parada numerada con posición y punto de mira `yaw`/`pitch`) o **`path`** (un **nodo de camino**: guía la ruta sin ser parada, para rodear muros → *pathfinding*). Compatible con proyectos anteriores.

**Edición (solo editor).** Se pulsa **«Editar recorrido»** (antes «Programar recorrido») y, caminando por la galería:
- **H** guarda un punto en la posición actual. **Por defecto es una posición** (parada numerada 1, 2, 3…). La tecla `g` no se usó porque ya sirve para agarrar/mover objetos.
- **Clic en un punto** lo selecciona y abre un panel en pantalla con su tipo: **Posición** (por defecto) ↔ **Nodo de camino**. Atajos con el puntero bloqueado: **T** cambia el tipo, **Supr** elimina, **H** **inserta** un punto nuevo **justo después** del seleccionado (para intercalar paradas o afinar el camino sin rehacer). Clic fuera cierra el panel.
- Botones «Deshacer punto» y «Vaciar recorrido».

**Marcadores en el suelo.** Cada posición se dibuja como un disco con su **número** (textura de canvas) y una **flecha** que indica hacia dónde mira; los nodos de camino son discos pequeños; entre puntos consecutivos se dibujan flechas de dirección. Un token bajo el **1** marca el inicio. Los marcadores se renderizan igual en el editor y en el exportado.

**Reproducción (Recorrido + exportado).** Al **acercarse a la posición 1** (con histéresis para no dispararse por accidente) el recorrido **arranca solo**; también se inicia pulsando **→** o haciendo clic en el marcador 1. **← →** avanzan y retroceden entre posiciones con **movimiento automático de cámara** que interpola posición y mirada y **sigue los nodos de camino** intermedios. **WASD** sale al modo libre. La lógica de reproducción (`tourGoTo`/`tourAnim`) es la ya probada; esta versión añade la numeración, la selección/inserción y el auto-inicio por proximidad.

**Verificación.** Sintaxis (`node --check`) de ambos motores; y una prueba de la **lógica de puntos** (anexar, **insertar entre dos**, numeración de posiciones ignorando los nodos de camino, cambio de tipo con renumeración, borrado con ajuste de selección, vaciado): **15/15**. Se mantienen en verde los tests de geometría (rotación, smoke con sala circular y escalera en U, y huecos no rectangulares / multi-escalera). La verificación **visual** con WebGL sigue fuera del entorno de construcción; conviene una pasada: crear 3–4 posiciones con H, intercalar un nodo de camino tras una muralla, y comprobar el auto-inicio y el avance con → en Recorrido y en el HTML exportado.

---

### 11.1 Colocación de obras en pisos superiores + obra que desaparecía al moverla (v2.11.1) — ✅
Dos fallos relacionados con las obras (imágenes y textos) en edificios de varias plantas:

**1) Los pisos superiores no detectaban las paredes.** `wallMeshes` (el conjunto de mallas de pared contra el que se lanza el rayo para colocar y detectar obras) solo se fijaba para el piso **activo al construir**; al subir de piso no se actualizaba. Por eso en los pisos superiores no se podían colocar cuadros ni textos (el rayo no encontraba pared) ni se detectaban bien. Arreglo: `wallMeshes`, `roomMats` y `corridorMat` se **guardan por piso** en `FLOORS[i]` y `setFloorCollision(i)` los **cambia** al pisar cada planta. Como al subir por la escalera ya se llama a `setFloorCollision(floorHi)`, ahora las paredes del piso correcto quedan activas y la colocación/detección funciona en todas las plantas.

**2) La obra desaparecía al subirla por el muro con G.** En `stick` (que pega la obra a la pared bajo la mira), el piso se elegía como el **más cercano** al punto de impacto. Al mover la obra hacia arriba, en cuanto el punto cruzaba el **punto medio entre dos plantas**, el piso elegido saltaba al **superior** y la obra se **teletransportaba** por encima del techo (parecía desaparecer). Ocurría igual con los textos, por compartir la función. Arreglo: `stick` ahora elige el piso como **el más alto cuya base no supere el punto tocado** (se queda en el muro que realmente se está tocando) y **acota la altura al propio muro**, de modo que la obra sube de forma continua y nunca salta de planta ni se coloca por encima del muro.

**Verificación.** Sintaxis de ambos motores; una prueba de la **lógica de `stick`** que recorre el muro del piso 0 (se mantiene en el piso 0 y acota la altura), contrasta con la lógica antigua (que saltaba al piso 1 pasado el punto medio) y comprueba la colocación en el muro del piso 1: **23/23**. El resto de tests (geometría, huecos, recorrido) siguen en verde. El motor exportado no necesitó cambios: su detección de obras usa oclusión contra toda la geometría, no `wallMeshes` por piso.
...
### 11.2 UX del recorrido en el HTML exportado: mensaje y controles táctiles (v2.11.2) — ✅
Ajustes de interfaz del recorrido guiado en el **HTML exportado** (sin cambios funcionales en la reproducción):

**Mensaje abajo a la izquierda.** El aviso del recorrido («Acércate al 1…» y el contador «Recorrido N/total») ya no aparece centrado, sino en un panel fijo **abajo a la izquierda** (`#tourMsg`), fuera del punto de mira y de los cuadros. El aviso de sentarse/levantarse sigue en su sitio.

**Botón «Iniciar recorrido» en móvil.** En dispositivos táctiles aparece un botón **▶ Iniciar recorrido** en la parte inferior **solo cuando el usuario apunta a los nodos del recorrido** (rayo desde el centro contra los marcadores) o se acerca a la posición 1. En escritorio se mantiene el inicio con **→**, clic en el marcador y el **auto-inicio por proximidad** (que en móvil se desactiva a favor del botón, para no arrancar sin querer).

**Controles en pantalla durante el recorrido (móvil).** Una vez iniciado, aparecen tres botones fijados en la **parte inferior**: **‹** (posición anterior), **Salir** (centro, termina el recorrido) y **›** (siguiente). Están en una barra con `pointer-events` selectivos y **safe-area** inferior, y el manejador táctil ignora los toques que caen sobre ellos, de modo que **no interfieren** con el joystick, el gesto de mirar ni la apertura de cuadros.

**Verificación.** Sintaxis de ambos motores; materialización del `TOUR_TEMPLATE` comprobando que los elementos (`#tourMsg`, `#tourStart`, `#tourNav`, botones y CSS `.tourbar`) aparecen en el export; y el resto de la batería (geometría, huecos, recorrido, `stick`) en verde. La verificación **visual/táctil** real conviene hacerla en un móvil: exportar el HTML, apuntar a un marcador para ver el botón, iniciar y comprobar ‹ / Salir / › abajo.

---

*Fin del documento — v2.11.2.*
